...
import { selectSelectedHotel } from '../../redux/booking/hotelBookingSlice';

const UpdateHotelForm = () => {
    // /hotelupdate?hId=${hId}&username=${name}&useremail=${email}&date1=${date1}&htl=${hotel_name}
    // &type=${hotel_type}&nguest=${no_of_guest}&chk_in=${check_in}&chk_out=${check_out}&price=${hotel_price}
    const [selectedHotel, setSelectedHotel] = useState({});
    const selectedBooking = useSelector(selectSelectedHotel);
    // name ,email , date1,hotel_name,hotel_type,
    // no_of_guest,check_in,check_out,hotel_price
  const [formData, setFormData] = useState({
  //   hId: new URLSearchParams(window.location.search).get("hId") || '',
  //   name:  new URLSearchParams(window.location.search).get("username") || '',
  //   email: new URLSearchParams(window.location.search).get("useremail") ||  '',
  //   date1: new URLSearchParams(window.location.search).get("date1") || '',
  //   hotel_name: new URLSearchParams(window.location.search).get("htl") ||  '',
  //   hotel_type: new URLSearchParams(window.location.search).get("type") ||  '',
  //  no_of_guest: new URLSearchParams(window.location.search).get("nguest") || '',
  // check_in: new URLSearchParams(window.location.search).get("chk_in") || '',
  // check_out: new URLSearchParams(window.location.search).get("chk_out") || '',
  //   hotel_price: new URLSearchParams(window.location.search).get("price") || '',
    hId: selectedBooking?._id || 'N/A',
    name:  selectedBooking?.name || 'N/A',
    email: selectedBooking?.email || 'N/A',
    date1: selectedBooking?.date1 || 'N/A',
    hotel_name: selectedBooking?.hotel_name || 'N/A',
    hotel_type: selectedBooking?.hotel_type || 'N/A',
   no_of_guest: selectedBooking?.no_of_guest || 'N/A',
  check_in: selectedBooking?.check_in || 'N/A',
  check_out: selectedBooking?.check_out || 'N/A',
    hotel_price: selectedBooking?.hotel_price || 'N/A',
  });
  ...
  useEffect(() => {
      setFormData(prevData => ({ ...prevData, hotel_price: hotelPrice }));
  }, [hotelPrice]); // Update formData when packagePrice changes
  ...
  const validateForm = () => {
    ...
    if (!formData.hId) {
        newErrors.hId = 'Hotel Id is required';
        isValid = false;
      }
    ...
    if (!formData.hotel_name === "--Select Hotel--") {
      newErrors.hotel_name = 'Hotel is required';
      isValid = false;
    }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/hotels/hotels-update', formData);
    ...
    <Row>
      <Col md={{ span: 6, offset: 3 }}>
        <Form onSubmit={handleSubmit}>
          <h3 style={{marginTop:20,marginBottom:20}}>Update Hotel Booking</h3>

          <Form.Group controlId="hId" style={{marginTop:20,marginBottom:20}}>
            <Form.Control
              type="text"
              placeholder="Enter Hotel Booking Id"
              value={formData.hId}
              onChange={(e) => setFormData({ ...formData, hId: e.target.value })}
              isInvalid={!!errors.hId}
            />
            <Form.Control.Feedback type="invalid">{errors.hId}</Form.Control.Feedback>
          </Form.Group>
